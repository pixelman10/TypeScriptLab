...
let person :{name:string, age:number, education:string,Isstudent:boolean}= {  //in curley breces is called type annotations.
    name:"usman",
    age:16,
    education:"Matric",
    Isstudent:true,
...
let myObject :{car:string, model:number , manufacturer:string, capacityseats:number}={
    car:"Toyota XLi ",
    model:2006,
    manufacturer:"Toyota",
    capacityseats:5,
...
//=======Nested Objects========:
let studentdata2:{
    name?:string, //it is optional key-value  pair
    age:number,
    isEnrolled:boolean,
    id:number,
...
let studentdata3:{name:string,age:number,isEnrolled:boolean,id:number,address:{city:string,country:string},[key:string]:any}={
    name:"Asad",
    age:17,
    isEnrolled:true,
    id:5002,
...
let academyFees:{english:number,math:number,physics:number,chemistry:number}={
    english:8000,
    math:10000,
    physics:9000,
    chemistry:9000,
